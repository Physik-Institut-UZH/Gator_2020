...
void GatorSampleAnalysis::doAnalysis(){
	...
	for(int iLine=0; iLine<nLines; iLine++){
		...
		if(mb_BGflag){
			...
			line->BgPeakCnts_err = sqrt(line->BgcountsS+1);//Added by Gabriela 06.2021
			...
			line->BgCompCnts_err = sqrt(line->BgcountsL+line->BgcountsR+1)*bg_line_interv/bg_comp_interv;//Added by Gabriela 06.2021
			...
			//line->BgLdCnts = 2.86 + 4.87*TMath::Sqrt(1.36 + line->BgCompCnts);//From the Gator paper;  //Removed by Gabriela 06.2021 - value is not relevant anymore			
			...
			//Full block below removed (Gabriela 07.2021)
			//if(line->BgLdCnts<line->BgLineCnts){
			/*if(line->BgLdCnts<=line->BgLineCnts){//added by Gabriela 06.2021. 
				line->BgDetected=true;
			}else{
				line->BgDetected=false;
			}*/
			// now, bg is always detected if if(mb_BGflag) // Gabriela 07.2021
			line->BgDetected=true;
		}else{
			line->BgDetected=false;
		}
		...
		//line->PeakCnts_err=sqrt(line->countsS);//changed by Gabriela 06.2021
		line->PeakCnts_err=sqrt(line->countsS+1);//added " "
		
		line->CompCnts = (line->countsL+line->countsR)*line_interv/comp_interv;
		//line->CompCnts_err = sqrt(line->countsL+line->countsR)*line_interv/comp_interv;//changed by Gabriela 06.2021
		line->CompCnts_err = sqrt(line->countsL+line->countsR+1)*line_interv/comp_interv;//added " "
		
		if(line->BgDetected){
			//double bgcnts = (md_aqtime/md_Bgtime)*line->BgLineCnts; changed by Gabriela 07.2021
			double bgcnts=0; // this will be used only to calculate net line counts
			if (line->BgLineCnts>=0) bgcnts = line->BgLineCnts; // it only takes a value if positive
			double bgcnts_err =line->BgLineCnts_err; // The error is always calculated

			double time_ratio=(md_aqtime/md_Bgtime);
	
			line->LineCnts = line->PeakCnts - line->CompCnts - bgcnts*time_ratio;
		...
		double BRxlow_limit_eff=BRxEff*low_limit_eff;
		
		//line->LdActiv = 1000*line->LdCnts/(md_quant*BRxEff*md_aqtime);
		line->LdActiv = 1000*line->LdCnts/(md_quant*BRxlow_limit_eff*md_aqtime);// now the systematic error on the eff is taken into account (G. 07.2021) 
		...
		line->Activ_err = line->Activ*sqrt( pow(system_error_eff,2) + pow(line->LineCnts_err/line->LineCnts,2) ); // now error is given in terms of system_error_eff // (G. 07.2021)
		...
		if(mb_BGflag){
			...
			//logfile << " Background detection limit = " << line->BgLdCnts << endl; // removed by Gabriela 07.2021
...
void GatorSampleAnalysis::WriteOutputTable(string outfilename){
	...
	outfile<<"\\begin{tabular}{||c c c c c c c c c c||}"<<endl;
	outfile<<"\\hline"<<endl;
	outfile<<"Isotope & E(keV) & PeakCnts & CompCnts & BkCnts & isBkdet & LineCnts & LdCnts & LdActiv & Activity (mBq/u.) \\\\"<<endl;
	outfile<<"\\hline"<<endl;
	for(int iLine=0; iLine<nLines; iLine++){
		LineStruct *line = GetLine(iLine);
		...
		if (line->BgDetected) {tmpstr="T";} else {tmpstr="F";}
		outfile << tmpstr << "  &  ";
