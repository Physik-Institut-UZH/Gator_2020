...
TH1F* convert_histo_ENR(TH1F*,Double_t,TF1*);
...
void Th_CalibScr(){
	...
	//Here follow the vector of the literature lines energies to look for and the corresponding uncertainties (to be loaded from an initfile):
	vector<Double_t>* ENR_litt = new vector<Double_t>;
	vector<Double_t>* ENR_litt_err = new vector<Double_t>;
	
	//ADC lower and upper limits of the ranges of the peaks (to be loaded from an init file):
	vector<Double_t>* ADC_low_range = new vector<Double_t>;
	vector<Double_t>* ADC_up_range = new vector<Double_t>;
	
	//Initialization values of the peak centers and width for the first fitting process (to be loaded from an init file):
	vector<Double_t>* ADC_tmp_centers = new vector<Double_t>;
	vector<Double_t>* ADC_tmp_sigmas = new vector<Double_t>;
...
//Creation of an histogram with the ASCII data of the Th228 source. Draw it in the second pad.
	...
	string filename("228Th_supported_20110816Data.txt");
...
//The lines to be searched for the calibration are (in ascending energy order):
	...
	//The file where are the info to initialize the fitting procedure for the peaks
	ifstream initfile;
	initfile.open("linesinit.txt");
	
	Double_t litenr, litenrerr, center, sigma, lowran, upran;
	while(initfile.good()){
		if(initfile.eof()) break;
		...
		ENR_litt -> push_back(litenr);
		ENR_litt_err -> push_back(litenrerr);
		ADC_tmp_centers -> push_back(center);
		ADC_tmp_sigmas -> push_back(sigma);
		ADC_low_range -> push_back(lowran);
		ADC_up_range -> push_back(upran);
...
//Temporary variables used for the multistep fit.
	...
	for (Int_t i=0;i<nlines;i++) {
		...
		//ROI selection
		h -> GetXaxis() -> SetRangeUser(ADC_low_range->at(i),ADC_up_range->at(i));
		ff_ADC -> SetRange(ADC_low_range->at(i),ADC_up_range->at(i));
		c1 -> Modified();
...
//STEP1: only gaussian -> other functions switched off.
		...
		ff_ADC -> SetParameter(0,ADC_tmp_centers->at(i)); //Gaussian Center initialization
		ff_ADC -> SetParameter(1,initAmpl(h)); //Amplitude
		ff_ADC -> SetParameter(3,ADC_tmp_sigmas->at(i)); //Sigma initialization!!! 
...
//STEP4: all parameters initialized, start the global fit
		...
		if (fitStatus==0){
			...
			litt_vec -> push_back(ENR_litt->at(i));
			litt_err_vec -> push_back(ENR_litt_err->at(i));
			ADClowRange_vec -> push_back(ADC_low_range->at(i));
			ADCupRange_vec -> push_back(ADC_up_range->at(i));
		}
		...
		gaus_fnc_vec[i] -> SetRange(ADC_low_range->at(i),ADC_up_range->at(i));
		...
		tail_fnc_vec[i] -> SetRange(ADC_low_range->at(i),ADC_up_range->at(i));
		...
		step_fnc_vec[i] -> SetRange(ADC_low_range->at(i),ADC_up_range->at(i));
		...
		lin_fnc_vec[i] -> SetRange(ADC_low_range->at(i),ADC_up_range->at(i));
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	TF1* calib_old_fcn_fw = new TF1("calib_old_fcn_fw","pol2");
	calib_old_fcn_fw -> SetLineColor(2);
	calib_old_fcn_fw -> SetLineWidth(1);
	
	
	resFit = gr_cal_fw -> Fit(calib_old_fcn_fw,"S");
	...
	TGraphErrors* gr_cal_old_res_fw = plotResiduals(calib_old_fcn_fw,ADCcenters,ADCcenters_err,litt_vec,litt_err_vec,resFit);
	gr_cal_old_res_fw -> GetXaxis() -> SetTitle("ADC");
	gr_cal_old_res_fw -> GetYaxis() -> SetTitle("Energy [KeV]");
	gr_cal_old_res_fw -> Draw("AP");
	
	TLine* zeroline_fw = new TLine(gr_cal_old_res_fw->GetXaxis()->GetXmin(),0,gr_cal_old_res_fw->GetXaxis()->GetXmax(),0);
	...
	TF1* calib_old_fcn_bw = new TF1("calib_old_fcn_bw","pol2");
	calib_old_fcn_bw -> SetLineColor(2);
	calib_old_fcn_bw -> SetLineWidth(1);
	
	resFit = gr_cal_bw -> Fit(calib_old_fcn_bw,"S");
	...
	//Using the function plotResiduals to generate the residual plot
	TGraphErrors* gr_cal_old_res_bw = plotResiduals(calib_old_fcn_bw,litt_vec,litt_err_vec,ADCcenters,ADCcenters_err,resFit);
	gr_cal_old_res_bw -> GetXaxis() -> SetTitle("Energy [KeV]");
	gr_cal_old_res_bw -> GetYaxis() -> SetTitle("ADC");
	gr_cal_old_res_bw -> Draw("AP");
	
	TLine* zeroline_bw = new TLine(gr_cal_old_res_bw->GetXaxis()->GetXmin(),0,gr_cal_old_res_bw->GetXaxis()->GetXmax(),0);
...
//Saving the whole work in a root file
	TFile* filesave = new TFile("calib_old.root","update");
	
	filesave -> WriteTObject(calib_old_fcn_fw,"calib_old_fcn_fw","Overwrite");
	filesave -> WriteTObject(calib_old_fcn_bw,"calib_old_fcn_bw","Overwrite");
	filesave -> WriteTObject(resol_ADC_func,"resol_ADC_func","Overwrite");
	filesave -> WriteTObject(gr_cal_old_res_fw,"gr_cal_old_res_fw","Overwrite");
	filesave -> WriteTObject(gr_cal_old_res_bw,"gr_cal_old_res_bw","Overwrite");
	...
	/*
	//Conversion of the histogram in energy domain (old method)
	c6 = new TCanvas("c6","228Th source calibration (KeV)");
	c6 -> cd();
	c6 -> SetLogy();
	
	Double_t first_enr_bin = calib_old_fcn_fw -> Eval(h->GetBinCenter(h->GetXaxis()->GetFirst()));
	Double_t last_enr_bin = calib_old_fcn_fw -> Eval(h->GetBinCenter(h->GetXaxis()->GetLast()));
	
	TH1F* h_ENR2 = new TH1F("h_ENR2","Energy calib",h->GetNbinsX(),first_enr_bin,last_enr_bin);
	
	for (Int_t i = h->GetXaxis()->GetFirst(); i <= h->GetXaxis()->GetLast(); i++) {
		h_ENR2 -> Fill(calib_old_fcn_fw->Eval(i*1.),h->GetBinContent(i));
	}
...
//Working on the energy domain to produce a spectrum and a plot to show. For everything work only in ADC domain energy domain is only for showing.
	...
	TH1F* h_ENR = convert_histo_ENR(h,time,calib_old_fcn_fw);
	h_ENR -> Draw(); 
	...
	c6 = new TCanvas("c6","Resolution plot (KeV)");
	c6 -> cd();
	//c6 -> SetLogy();
	
	
	//Plot for the resolution function fitting.
	TGraphErrors* gr_resol_ENR = new TGraphErrors(nlines,&(ENRcenters->at(0)),&(ENRwidths->at(0)),&(ENRcenters_err->at(0)),&(ENRwidths_err->at(0)));
	gr_resol_ENR -> Draw("AP");
	
	TF1* resol_ENR_func = new TF1("resol_ENR_func",resolFunc,h_ENR->GetBinCenter(h_ENR->GetXaxis()->GetFirst()),h_ENR->GetBinCenter(h_ENR->GetXaxis()->GetLast()),3);
	
	ENRresolFncInit(resol_ENR_func,ENRcenters,ENRwidths);
	resol_ENR_func -> SetLineColor(2);
	resol_ENR_func -> SetLineWidth(1);
	
	gr_resol_ENR -> Fit(resol_ENR_func);
	
	gr_resol_ENR -> GetXaxis()->SetTitle("Energy [KeV]");
	gr_resol_ENR -> GetYaxis()->SetTitle("Peak width [KeV]");
	
	
	
	filesave = new TFile("calib_old.root","update");
	
	filesave -> WriteTObject(resol_ENR_func,"resol_ENR_func","update");
...
TH1F* convert_histo_ENR(TH1F* hADC, Double_t time, TF1* calib_fcn_fw){
