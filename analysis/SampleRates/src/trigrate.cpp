...
TGraphErrors* trigrate::MakeFullPlot(){
	...
	pgraph->SetLineColor(2);
    pgraph->SetMarkerColor(2);
    pgraph->SetMarkerStyle(kFullCircle);
	
	for(int i=0; i<m_ptimestamp->size(); i++){
		
//		pgraph -> SetPoint(point,(Double_t)m_ptimestamp->at(i),m_ptrigrate->at(i));
		pgraph -> SetPoint(point,(i + 1.5)/2,43200*(m_ptrigrate->at(i)));
		pgraph -> SetPointError(point,0,43200*(m_ptrigrate_err->at(i)));
		point++;
	}
	pgraph->GetXaxis()->SetTitle("Time (days)");
	pgraph->GetYaxis()->SetTitle("Total counts");
